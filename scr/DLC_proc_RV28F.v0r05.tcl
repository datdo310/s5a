...
# DLC_proc_RV28F.tcl
#
# v0r03: 2019/01/10 Change Initial Transition 9999.999->99999.99
# v0r04: 2020/08/19 Remove min_period and half cycle section
#                   Add RV28F TSMC
# v0r05: 2020/10/02 Adjust 7Tr cell driverability based on W size
############################################################
############################################################
...
set RV28FT_MAXTRAN_LOWDRV_Vall_Fall_SVT_X20 [expr $Tfactor*0.60*$DLCLT_MAXTRAN_LOWDRV_MARGIN]
set RV28FT_MAXTRAN_LOWDRV_Vall_Fall_ULVT_X10 [expr $Tfactor*0.70*$DLCLT_MAXTRAN_LOWDRV_MARGIN]
...
proc RV28FT_set_maxtran_lowdrv_Vall_Fall_SVT { pin drive overwrite } {
    ...
    if { $drive <= 20 } {
        if { $overwrite == 1 } {
            set_max_transition $::RV28FT_MAXTRAN_LOWDRV_Vall_Fall_SVT_X20 $pin
            set DLCLT_SET_MAXTRAN_LOWDRV_FLAG 1
        } else {
            set old_const [get_attribute -quiet $pin max_transition]
            if { $old_const == "" } {
            ...
            if { $::RV28FT_MAXTRAN_LOWDRV_Vall_Fall_SVT_X20 < $old_const } {
                set_max_transition $::RV28FT_MAXTRAN_LOWDRV_Vall_Fall_SVT_X20 $pin
                set DLCLT_SET_MAXTRAN_LOWDRV_FLAG 1
            }
        }
...
proc RV28FT_set_maxtran_lowdrv_Vall_Fall_ULVT { pin drive overwrite } {
    ...
    if { $drive <= 10 } {
        if { $overwrite == 1 } {
            set_max_transition $::RV28FT_MAXTRAN_LOWDRV_Vall_Fall_ULVT_X10 $pin
            set DLCLT_SET_MAXTRAN_LOWDRV_FLAG 1
        } else {
            set old_const [get_attribute -quiet $pin max_transition]
            if { $old_const == "" } {
            ...
            if { $::RV28FT_MAXTRAN_LOWDRV_Vall_Fall_ULVT_X10 < $old_const } {
                set_max_transition $::RV28FT_MAXTRAN_LOWDRV_Vall_Fall_ULVT_X10 $pin
                set DLCLT_SET_MAXTRAN_LOWDRV_FLAG 1
            }
        }
