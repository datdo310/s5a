...
# v2r50 : 2022/03/23 : Revise chkGCLKPath proc
# v2r51 : 2022/04/13 : GET_STARTEND_CYCLE updates digit %.4f
# v2r52 : 2022/05/13 : Add proc for clock_crossing for PTC
#                    : Update COL2DISP for sort_colletion
#                    : Update chkGCLKPath for reject duplicated report
#################################################################
...
proc COL2DISP { {COLLECTION} } {
    if {[sizeof_collection $COLLECTION] == 0} {return {}}
    set COLLECTION_sort [sort_collection -dictionary $COLLECTION full_name]
    foreach_in_collection tmp $COLLECTION_sort {
...
proc chk_path_cycle {source endpoint startclk endclk delay return_val} {
    ...
    if {[sizeof_collection $paths] == 0} {
        ...
        set path_period  [expr $t_end_edge - $t_start_edge]
        set path_cycle   [expr ($t_end_edge - $t_start_edge) / $min_period]
        if {$return_val == "period"} {
            return [format "%.3f" $path_period]
        } elseif {$return_val == "cycle"} {
...
proc chkGCLKPathAll {{type "all"}} {
    if {[sizeof_collection [get_clocks -q -filter "is_generated==true"]]==0} {
        puts "* Information: This design has no Generated clock."
        return
    }
        ...
        foreach_in_collection source $sources {
            ...
            if {$type=="all"} {
                if {[get_attribute $source object_class]=="port"} {
                    ...
                    puts "# Generated clock at PORT"
                    continue
                } else {
                    set source_clocks [lsort -u [get_attribute [get_attribute [get_pins -q -of [get_cells -of $source] -filter "defined(clocks)"] clocks] full_name]]
                }
            } else {
            ...
            if {[sizeof_collection $gclk_fanout] == 0} {
                ...
                foreach_in_collection pin $gclk_fanout_to_data {
                    puts "# SetupCycle HoldCycle SetupPeriod HoldPeriod sync/async : Startpin (StartCLK StartCLKPeriod) -> Endpin (EndCLK EndCLKPeriod)"
                    ...
                    foreach clk1 $source_clocks {
                        foreach clk2 $endpoint_clocks {
                            if {![info exist line{$source_name,$endpoint,$clk1,$clk2}]} {
                                set line{$source_name,$endpoint,$clk1,$clk2} 1
                                puts " [chk_path_cycle $source_name $endpoint $clk1 $clk2 "max" "cycle"] [chk_path_cycle $source_name $endpoint $clk1 $clk2 "min" "cycle"] [chk_path_cycle $source_name $endpoint $clk1 $clk2 "max" "period"] [chk_path_cycle $source_name $endpoint $clk1 $clk2 "min" "period"] [chk_sync_clock $clk1 $clk2] : ${source_name} ($clk1 [get_attribute [get_clocks $clk1] period]) -> ${endpoint} ($clk2 [get_attribute [get_clocks $clk2] period])"
                            }
                        }
                    }
                }
...
proc GET_STARTEND_CYCLE { COLLECTION REPFILE STCLOCKFILE EDCLOCKFILE } {
	...
	set CYCLE_TIME [format %.4f $CYCLE_TIME]
...
# used in gca_main.mcu.tcl
proc PTC_report_inverted_clock_crossing {REPORT_FILE} {
    set fid [FILE_OPEN $REPORT_FILE]
    set num 1
    while {[gets $fid line] >= 0} {
        if {$num == 1} {
            incr num
            ...
            set elm [split $line ","]
            set launchClock [lindex $elm 0]
            set launchClockEdge [lindex $elm 2]
            set captureClock [lindex $elm 3]
            set captureClockEdge [lindex $elm 5]
            set attribute [split [lindex $elm 7] ";"]
            if {[lsearch $attribute "F"] >= 0} {continue}
            if {$launchClockEdge=="R" && $captureClockEdge=="F"} {
                #debug#puts "$num $launchClock $launchClockEdge $captureClock $captureClockEdge $attribute"
                redirect tmp.inverted_analyze_path_${::MODE}_${num}.rep {
                    analyze_paths -rise_from [get_clocks $launchClock] -fall_to [get_clocks $captureClock] -path_type summary -max_endpoints 100000 -nosplit -text_only
                }
                incr num
            } elseif {$launchClockEdge=="F" && $captureClockEdge=="R"} {
                #debug#puts "$num $launchClock $launchClockEdge $captureClock $captureClockEdge $attribute"
                redirect tmp.inverted_analyze_path_${::MODE}_${num}.rep {
                    analyze_paths -rise_from [get_clocks $launchClock] -fall_to [get_clocks $captureClock] -path_type summary -max_endpoints 100000 -nosplit -text_only
                }
    ...
    close $fid
    puts "StartClock,StartPin,EndPin,EndClock,PathType"
    for {set i 2} {$i < $num} {incr i} {
        exec -ignorestderr perl ./bin/GCA_conv_analyze_path_rpt2csv.pl < tmp.inverted_analyze_path_${::MODE}_${i}.rep > tmp.inverted_analyze_path_${::MODE}_${i}.csv
        set fid [open tmp.inverted_analyze_path_${::MODE}_${i}.csv]
        while {[gets $fid line] >= 0} {
            if {![regexp ",CLOCK_GROUP" $line] && ![regexp ",FALSE" $line]} {
                puts "$line"
            }
        }
        close $fid
        exec rm tmp.inverted_analyze_path_${::MODE}_${i}.rep
        exec rm tmp.inverted_analyze_path_${::MODE}_${i}.csv
    }
}

proc PTC_report_different_clock_crossing {REPORT_FILE} {
    exec tail -n +2 $REPORT_FILE > tmp.body
    exec sort -k 11n -t , tmp.body > tmp.body.sort
    set fid [FILE_OPEN tmp.body.sort]
    set num 1
    while {[gets $fid line] >= 0} {
        set elm [split $line ","]
        set launchClock [lindex $elm 0]
        set launchClockPeriodWaveform [lindex $elm 1]
        set launchClockPeriod [lindex $launchClockPeriodWaveform 0]
        set launchClockEdge [lindex $elm 2]
        set captureClock [lindex $elm 3]
        set captureClockPeriodWaveform [lindex $elm 4]
        set captureClockPeriod [lindex $captureClockPeriodWaveform 0]
        set captureClockEdge [lindex $elm 5]
        if {($launchClockEdge=="R" && $captureClockEdge=="R") && ($launchClockPeriod!=$captureClockPeriod)} {
            redirect tmp.dif_clock_analyze_path_${::MODE}_${num}.rep {
                analyze_paths -rise_from [get_clocks $launchClock] -rise_to [get_clocks $captureClock] -path_type summary -max_endpoints 100000 -nosplit -text_only
            }
            incr num
        } elseif {($launchClockEdge=="F" && $captureClockEdge=="F") && ($launchClockPeriod!=$captureClockPeriod)} {
            redirect tmp.dif_clock_analyze_path_${::MODE}_${num}.rep {
                analyze_paths -fall_from [get_clocks $launchClock] -fall_to [get_clocks $captureClock] -path_type summary -max_endpoints 100000 -nosplit -text_only
            }
            incr num
        }
    }    
    close $fid
    exec rm tmp.body
    exec rm tmp.body.sort
    puts "StartClock,StartPin,EndPin,EndClock,PathType"
    for {set i 1} {$i < $num} {incr i} {
        exec -ignorestderr perl ./bin/GCA_conv_analyze_path_rpt2csv.pl < tmp.dif_clock_analyze_path_${::MODE}_${i}.rep > tmp.dif_clock_analyze_path_${::MODE}_${i}.csv
        set fid [open tmp.dif_clock_analyze_path_${::MODE}_${i}.csv]
        while {[gets $fid line] >= 0} {
            if {![regexp ",MULTICYCLE" $line] && ![regexp ",CLOCK_GROUP" $line] && ![regexp ",FALSE" $line]} {
                puts "$line"
            }
        }
        close $fid
        exec rm tmp.dif_clock_analyze_path_${::MODE}_${i}.rep
        exec rm tmp.dif_clock_analyze_path_${::MODE}_${i}.csv
    }
}
