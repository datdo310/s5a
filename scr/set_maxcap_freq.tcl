...
## 2020/10/15 v01r04 Y.Oda Add MF3 SignOff
...
proc set_maxcap_per_freq_CORE {
    ...
    { process RV40F } 
        ...
        if {[string match "RV40F" $process]} {
                puts "* Information: RV40F MaxCapacitance for Frequency are set"
                set_maxcap_per_freq_CORE_RV40F $c_list $Tfactor $Cfactor $freq_margin
        } elseif {[string match "RV28F" $process]} {
                puts "* Information: RV28F MaxCapacitance for Frequency are set"
                set_maxcap_per_freq_CORE_RV28F $c_list $Tfactor $Cfactor $freq_margin
        } elseif {[string match "MF3" $process]} {
                puts "* Information: MF3 MaxCapacitance for Frequency are set"
                set_maxcap_per_freq_CORE_MF3 $c_list $Tfactor $Cfactor $freq_margin
        }
}
...
proc set_maxcap_per_freq_CORE_MF3 {
    c_list
    Tfactor
    Cfactor
    freq_margin
...
#
	set clock_const 0.4
	set data_const  0.8
        set_max_capacitance $clock_const [all_clocks] -clock_path -rise -fall
        set_max_capacitance $data_const  [all_clocks] -data_path -rise -fall
}
