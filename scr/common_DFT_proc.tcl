...
proc getUCFaultPins {{FAULT_FILE}} {
	set return_value ""
	set nu 0
		...
		if {$tmp=="0"||$tmp=="1"} {
			...
			set value [lindex $str 0]
		    if {$chk == "UC" } {
				incr nu
				regsub "/" [lindex $str 2] "" pin
				lappend return_value "$pin $value"
				set oldchk $chk
		    } elseif {$oldchk == "UC" && $chk == "EQ"} {
				incr nu
				regsub "/" [lindex $str 2] "" pin
				lappend return_value "$pin $value"
		    } else {
        ...
        close $fid
	puts "* Total $nu number of uncontrol fault points are catched."
	return $return_value
}
...
proc removeUndetectNumber {} {
	...
	define_user_attribute -type int -class pin sum_undetect
	remove_user_attribute [get_pins -h *] sum_undetect -q
}
proc removeUncontrolNumber {} {
	define_user_attribute -type int -class pin uncontrol
	remove_user_attribute [get_pins -h *] uncontrol -q

	define_user_attribute -type int -class pin uncontrolvalue
	remove_user_attribute [get_pins -h *] uncontrolvalue -q

	define_user_attribute -type int -class pin sum_uncontrol
	remove_user_attribute [get_pins -h *] sum_uncontrol -q
}
proc setUncontrolNumber {{list_uncontrol}} {
	define_user_attribute -type int -class pin uncontrol
	define_user_attribute -type int -class pin uncontrolvalue
	set nu 0
	set nu_exist 0
	set return_value ""
	foreach tmp $list_uncontrol {
		incr nu
		set pin   [lindex $tmp 0]
		set value [lindex $tmp 1]
		
		set col_pin [get_pins $pin -q]
		if {[sizeof_collection $col_pin] == 0} {continue}
		lappend return_value $pin
		incr nu_exist
		set flg [get_attribute $col_pin uncontrol -quiet]
		if {$flg > 0} {
			set_user_attribute $col_pin uncontrol [expr $flg + 1] -quiet
		} else {
			set_user_attribute $col_pin uncontrol 1 -quiet
		}

		set vflg [get_attribute $col_pin uncontrolvalue -quiet]
		if {$vflg == ""} {
			set_user_attribute $col_pin uncontrolvalue $value -quiet
		} else {
			if {$vflg != $value} {
				set_user_attribute $col_pin uncontrolvalue Both -quiet
			}
		}
	}
...
proc minimizeUndetectTargetRange { {col_undetect} {max_num} {min_num} } {
	if {$max_num == "inf"} {
		set list_workpin [COL2LIST [get_pins $col_undetect -filter "sum_undetect>$min_num"]]
	} else {
		set list_workpin [COL2LIST [get_pins $col_undetect -filter "sum_undetect<=$max_num&&sum_undetect>$min_num"]]
	}
	foreach pin $list_workpin {
...
proc minimizeUncontrolTargetRange { {col_uncontrol} {max_num} {min_num} } {
	if {$max_num == "inf"} {
		set list_workpin [COL2LIST [get_pins $col_uncontrol -filter "sum_uncontrol>$min_num"]]
	} else {
		set list_workpin [COL2LIST [get_pins $col_uncontrol -filter "sum_uncontrol<=$max_num&&sum_uncontrol>$min_num"]]
	}
	foreach pin $list_workpin {
		set redundant_target [remove_from_collection [all_fanin  -to  $pin] [get_pins $pin]]
		set col_uncontrol    [remove_from_collection $col_uncontrol $redundant_target]

		set redundant_target [remove_from_collection [all_fanout -from $pin] [get_pins $pin]]
		set col_uncontrol    [remove_from_collection $col_uncontrol $redundant_target]
	}
	return $col_uncontrol
}

proc minimizeUncontrolTarget {{list_uncontrol}} {
	set return_value ""
	set col_uncontrol     [get_pins $list_uncontrol -q]
	set col_uncontrol_out [filter_collection $col_uncontrol pin_direction=="out"]
	set col_redun_in      [get_pins -of [get_nets -of $col_uncontrol_out] -leaf -filter pin_direction=="in" -q]
	set col_uncontrol     [remove_from_collection $col_uncontrol $col_redun_in]
	set list_uncontrol    [COL2LIST $col_uncontrol]
	#set list_uncontrol    [lsort -dictionary -ascii -unique $list_uncontrol]
	#set col_uncontrol     [get_pins $list_uncontrol]

	set col_uncontrol     [minimizeUncontrolTargetRange $col_uncontrol inf   10000]
	set j 10000
	for {set i  9000} {$i >= 3000 } { set i [expr $i-1000] } {
		set col_uncontrol     [minimizeUncontrolTargetRange $col_uncontrol $j  $i]
		set j $i
	}
	for {set i  2500} {$i >= 1000 } { set i [expr $i-500] } {
		set col_uncontrol     [minimizeUncontrolTargetRange $col_uncontrol $j  $i]
		set j $i
	}
	for {set i   900} {$i >=  300 } { set i [expr $i-50] } {
		set col_uncontrol     [minimizeUncontrolTargetRange $col_uncontrol $j  $i]
		set j $i
	}
	for {set i   280} {$i >=  100 } { set i [expr $i-20] } {
		set col_uncontrol     [minimizeUncontrolTargetRange $col_uncontrol $j  $i]
		set j $i
	}
	for {set i    90} {$i >=  30 } { set i [expr $i-5] } {
		set col_uncontrol     [minimizeUncontrolTargetRange $col_uncontrol $j  $i]
		set j $i
	}
	for {set i    25} {$i >=  3 } { set i [expr $i-1] } {
		set col_uncontrol     [minimizeUncontrolTargetRange $col_uncontrol $j  $i]
		set j $i
	}

	set return_value [COL2LIST $col_uncontrol]
	return $return_value
}
...
proc minimizeUndetectTarget {{list_undetect}} {
	...
	#set list_undetect    [lsort -dictionary -ascii -unique $list_undetect]
	#set col_undetect     [get_pins $list_undetect]

	set col_undetect     [minimizeUndetectTargetRange $col_undetect inf   10000]
	set j 10000
	for {set i  9000} {$i >= 3000 } { set i [expr $i-1000] } {
		set col_undetect     [minimizeUndetectTargetRange $col_undetect $j  $i]
		set j $i
	}
	for {set i  2500} {$i >= 1000 } { set i [expr $i-500] } {
		set col_undetect     [minimizeUndetectTargetRange $col_undetect $j  $i]
...
proc chkUncontrolNumber {{list_uncontrol}} {
	foreach pin $list_uncontrol {
		set num   [sum [get_attribute  [all_fanout -from $pin] uncontrol]]
		set onum  [sum [get_attribute  [all_fanin  -to $pin] uncontrol]]
		set value [get_attribute  [get_pins $pin] uncontrolvalue]
		puts "[expr $num+$onum] $value $pin"
	}
}

proc setSumUndetectNumber {{list_undetect}} {
	foreach pin $list_undetect {
		set num [sum [get_attribute  [all_fanin -to $pin] undetect]]
		set_user_attribute [get_pins $pin]  sum_undetect $num  -quiet
	}
}


proc setSumUncontrolNumber {{list_uncontrol}} {
	foreach pin $list_uncontrol {
		set num  [sum [get_attribute  [all_fanout -from $pin] uncontrol]]
		set onum [sum [get_attribute  [all_fanin  -to $pin] uncontrol]]
		set_user_attribute [get_pins $pin]  sum_uncontrol [expr $num + $onum]  -quiet
	}
}
...
proc chkUndetectNumber_auto {{FAULT_FILE}} {
	...
	setSumUndetectNumber  $list_undetect
	set list_undetect [minimizeUndetectTarget $list_undetect]
	chkUndetectNumber $list_undetect
	unsuppress_message {UIAT-4 ATTR-3}
}

proc chkUncontrolNumber_auto {{FAULT_FILE}} {
	suppress_message {UIAT-4 ATTR-3}
	removeUncontrolNumber
	set list_uncontrol [getUCFaultPins $FAULT_FILE]
	set list_uncontrol [lsort -dictionary -ascii -unique $list_uncontrol]
	set list_uncontrol [setUncontrolNumber $list_uncontrol]
	set list_uncontrol [lsort -dictionary -ascii -unique $list_uncontrol]
	setSumUncontrolNumber  $list_uncontrol
	set list_uncontrol [minimizeUncontrolTarget $list_uncontrol]
	chkUncontrolNumber $list_uncontrol
	unsuppress_message {UIAT-4 ATTR-3}
}
