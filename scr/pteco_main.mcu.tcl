...
# v0r27 : 2020/03/12 : REL/Manaka : support PT2017.12
# v0r27a: 2020/04/30 : REL/Manaka : set tcl_precision (6)
# v0r28 : 2020/07/22 : REL/Manaka : Change fix_eco_leakage to fix_eco_power, tcl_precision 6 to 12
# v0r29 : 2020/10/28 : Y.Oda      : Add CCE
# v0r30 : 2021/02/19 : REL/Manaka : Pass eco_strict_pin_name_equivalence and timing_save_pin_arrival_and_required to slave
# v0r31 : 2021/03/03 : REL/Manaka : Add setting of core-based license for PT2016.12 and PT2017.12 version.
# v0r32 : 2021/07/12 : REL/Manaka : Change to remote execute for read POWER_ATTR.ptsc
# v0r33 : 2021/07/21 : REL/Manaka : Enable selecting CPUs/MEM each hosts      
# v0r34 : 2021/07/30 : REL/Manaka : Add function and enable get report after pteco
# v0r35 : 2021/09/24 : REL/Manaka : Adupt using PhysicalAware PTECO
# v0r36 : 2022/01/27 : REL/Manaka : Obsolete PT VERSION check
#                
##############################################
...
#-----------------------------------------------------
SET_INIT_VAR ENABLE_READ_SDC              true;
SET_INIT_VAR CANCEL_TRANnCAP_REPORT       true;
SET_INIT_VAR CANCEL_REPORT_TIMING         true;
SET_INIT_VAR CANCEL_CONST_REPORT          true;
SET_INIT_VAR CANCEL_UPDATE_TIMING         true;
SET_INIT_VAR CANCEL_AC_REPORT             true;
SET_INIT_VAR REPORT_LESSER_THAN_ONLY      true;
...
remote_execute {
    ...
    # SET_PROHIBIT_HM_PINS {debug on/off -> 1/0} {STD_LIBCELL_NAME} {outpin on/off -> 1/0}
    SET_PROHIBIT_HM_PINS 0 "RV28F7T40P140*F_*" 1
...
if { $PTECO_FIX_SETUP == "true" } {
    ...
    if {$PTECO_PHYSICAL_AWARE == "true" && [info exist pteco_setup_bufferList]} {
        ...
        eval "fix_eco_timing -type setup -verbose $PTECO_SETUP_OPT -methods {size_cell insert_buffer} -buffer_list {$pteco_setup_bufferList} -physical_mode open_site"
    } else {
...
if {$PTECO_FIX_HOLD == "true"} {
    ...
    if {$PTECO_PHYSICAL_AWARE == "false"} {
        ...
        eval "fix_eco_timing -type hold -buffer_list {$pteco_hold_bufferList} -verbose $PTECO_HOLD_OPT"
        ...
        puts "* Information: HOLD_OPTION:$PTECO_HOLD_OPT -physical_mode open_site -buffer_list {$pteco_hold_bufferList}"
        eval "fix_eco_timing -type hold -physical_mode open_site -buffer_list {$pteco_hold_bufferList} -verbose $PTECO_HOLD_OPT"
        if {$PTECO_SPLIT_ECOCARD == "true"} {
            remote_execute {
        ...
        puts "* Information: HOLD_OPTION:$PTECO_HOLD_OPT -physical_mode occupied_site -buffer_list {$pteco_hold_bufferList}"
        eval "fix_eco_timing -type hold -physical_mode occupied_site -buffer_list {$pteco_hold_bufferList} -verbose $PTECO_HOLD_OPT"
        if {$PTECO_SPLIT_ECOCARD == "true"} {
            remote_execute {
...
if {$PTECO_FIX_HOLD_CCE == "true"} {
    ...
    puts "* Information: HOLD_OPTION:$PTECO_HOLD_OPT"
    eval "fix_eco_hold_timing -buffer_list {$pteco_hold_bufferList} -verbose $PTECO_HOLD_OPT"
