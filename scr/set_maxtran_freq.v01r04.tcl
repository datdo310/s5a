...
## 2020/10/15 v01r03 Y.Oda	Add MF3 SignOff
## 2021/02/09 v01r04 Y.Oda	Update MF3
...
proc set_maxtran_per_freq_CORE {
	...
	if {[string match "RV40F" $process]} {
		...
		puts "Information: MF3 MaxTransition for Frequency are set"
		set_maxtran_per_freq_CORE_MF3 $c_list $Tfactor $Cfactor $freq_margin
	}
}
...
## MF3 Frequency Transition Procedure
proc set_maxtran_per_freq_CORE_MF3 {
    c_list
    Tfactor
    Cfactor
    freq_margin
...
#------------------------------------------------------------
# RANGE  : T >= 25ns (40MHz)
#   MaxTran(clock_path) = 0.0*T + 2.5 [ns]
#   MaxTran(data_path)  = 0.0*T + 5.0 [ns]
# RANGE  : T <  25ns
#   MaxTran(clock_path) = 0.10*T + 0 [ns]
#   MacTran(data_path)  = 0.20*T + 0.0 [ns]

    puts "Information: set_maxtran_per_freq_CORE_MF3"

    set TMAX1 [expr 25  * $Tfactor]
    foreach_in_collection clk $c_list {
        set period_tmp_tmp [get_attribute $clk period]
        set period_tmp [expr $period_tmp_tmp / (1 - $freq_margin) ]
        if { $period_tmp == "" } { continue }
        if { $period_tmp >= $TMAX1 } {
            set clock_const [expr $Cfactor*($period_tmp*0.0+2.5)]
            set data_const  [expr $Cfactor*($period_tmp*0.0+5.0)]
            set_max_transition $clock_const $clk -clock_path -rise -fall
            set_max_transition $data_const  $clk -data_path  -rise -fall
        } elseif { $period_tmp < $TMAX1 } {
            set clock_const [expr $Cfactor*($period_tmp*0.10+0.0)]
            set data_const  [expr $Cfactor*($period_tmp*0.20+0.0)]
            set_max_transition $clock_const $clk -clock_path -rise -fall
            set_max_transition $data_const  $clk -data_path  -rise -fall
        } else {
            puts "Error(transition setting): unconstrained for maxtran freq for [get_object_name $clk]"
