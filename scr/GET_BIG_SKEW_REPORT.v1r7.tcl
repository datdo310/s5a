...
#
# Version : v1r7 2020/11/19 : Skip recover502 which doesn't work expectedly
# Version : v1r6 2020/04/24 : Add startpoint output
# Version : v1r5 2014/08/25
#          (1) changed group option. Remove get_object_name
...
proc WRITE_BIGSKEW_REPORT { {CLOCK} {REP} {LIMIT_SKEW} {MODE} {DIR} {SLACK_MARGIN} {SLACK_MARGIN_L -999}} {
	...
	foreach_in_collection rep $REP {
		...
		if {$SKEW >= $LIMIT_SKEW} {
			set ST_PIN  [get_object_name [get_attribute $rep startpoint]]
			set END_PIN [get_object_name [get_attribute $rep endpoint]]
			echo [format "%30s slack: %5.3f skew(crpr): %5.3f(%5.3f) %s %s" $CLOCK $SLACK $SKEW $CRPRabs $ST_PIN $END_PIN] >> $DIR/log.$MODE.$log_clock
...
proc GET_BIG_SKEW_REPORT {{LIMIT_SKEW "0.3"} {SLACK_MARGIN "0.1"} {NWORST "1"} {DIR "./BigSkew"}} {
	...
	foreach f [glob -directory $TMP_DIR "log.*"] {
		puts "* Information : Checking UITE-502 in $f"
		set fid [open $f r]
		while {[gets $fid str] >= 0} {
			if {[regexp {(UITE-502)} $str]} {
				puts "* Information : UITE-502 was found in $f"
			}
		}
		close $fid
...
proc recover502 {{file} {NWORST} {DEPTH} {ROOT_DIR}} {
	...
	#
	set TMG_OPT "-delay min -unique_pins -path full_clock_expanded -nworst $NWORST -max 2000000"
	if {![regexp {2012.} ${::sh_product_version}]} {
		set TMG_OPT "${TMG_OPT} -group \[get_path_groups \*\]"
	}
	set stringCMD "parallel_execute \{\n"
