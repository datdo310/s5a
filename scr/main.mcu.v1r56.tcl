...
#          : mcu.v1r56 2021/02/19 Add keep pin name equivalent setting
...
SET_INIT_VAR XT_DD_MIN_SLACK          0.0001;# Hold xtalk delta check is done , if the slack < XT_DD_MIN_SLACK
...
if {![info exists NUM_SLACK]}  { set NUM_SLACK    0.0001 }
...
if {$STA_MODE == "mkSDF"} {
	...
	puts "* Information : Any OCV settings are skipped in this mode."
	...
	if {$ENABLE_OCV=="true"} {
		...
		if {![array exists ocv_param_list]} {
			puts "* Error : No OCV paramater table \"all_ocv_list\" found in design.cfg."
			exit
		}
		...
		if {[info exists ocv_param_list(${CONDITION},${DELAY})]} {
			...
			puts "* Information : No OCV sta mode\"."
		} else {
			puts "* Error : No OCV parameter found for \"${CONDITION} ${DELAY}\"."
			exit
		}
		# << OCV setting for special library >>
		if {[info exists special_ocv_param_table]} {
			puts "* Information : special ocv setting found."
			READ_SPECIAL_OCV
			if {![info exists special_ocv_param_list(${CONDITION},${DELAY})]} {
				puts "* Information  : No OCV parameter found for \"${CONDITION} ${DELAY}\"."
			}
...
if {$ENABLE_WRITE_SIMSDF == "true"} {
	...
	check_resource Ganerating_SDF_START
        ...
        #reset OCV
        set_timing_derate  -early 1.000 -cell_delay 
        set_timing_derate  -late  1.000 -cell_delay
        set_timing_derate  -early 1.000 -cell_delay [get_lib_cells */*]
        set_timing_derate  -late  1.000 -cell_delay [get_lib_cells */*]
        set_timing_derate  -early 1.000 -net_delay -static 
        set_timing_derate  -late  1.000 -net_delay -static
        set_timing_derate  -early 1.000 -net_delay -dynamic
        set_timing_derate  -late  1.000 -net_delay -dynamic
	...
	puts "* Warning     : Delay & OCV were changed so you cannot get STA report after this."
	puts "                So PrimeTime was terminated mandatory."
        print_message_info
        exit
} else {
