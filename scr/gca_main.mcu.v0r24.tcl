...
# v0r23 : 2022/01/05 : T.Manaka : fixed exec proc "get_pseudo_error_of_DES0002", Change option of summary report
# v0r24 : 2022/04/27 : REL/Manaka : add ENABLE_REPORT_CLOCK_CROSSING
#
################################################################
...
# Make clock crossing report
SET_INIT_VAR ENABLE_REPORT_CLOCK_CROSSING          false ;# true->make report_clock_crossing
...
if { ${PTC_USE_WAIVER_RULE} == "true" } {
    ...
    foreach tmp ${PTC_WAIVER_RULE_FILE} {
        ...
        if {${tmp} == "${PTC_WAIVER_RULE_DIR}/remove_DES0002_noise.tcl"} {
            puts "* Information: get_pseudo_error_of_DES0002"
            get_pseudo_error_of_DES0002
        }
    }
}
...
if {![info exists DEBUG_JOB]} {
    ...
    # << clock crossing report >>
    if {$ENABLE_REPORT_CLOCK_CROSSING == "true"} {
        puts "* Information: Start clock crossing report."
        check_resource START_clock_crossing_report
        report_clock_crossing -nosplit -verbose -period -edge_relationships -format csv -output ${REPORT_DIR}/clock_crossing_${MODE}.csv
        if {$ENABLE_INVERTED_CLOCK_CROSSING == "true"} {
            puts "* Information: Start inverted clock crossing report."
            redirect $REPORT_DIR/inverted_clock_path_${MODE}.csv {
                PTC_report_inverted_clock_crossing ${REPORT_DIR}/clock_crossing_${MODE}.csv
            }
        }
        if {$ENABLE_DIFFERENT_CLOCK_CROSSING == "true"} {
            puts "* Information: Start different clock crossing report."
            redirect $REPORT_DIR/different_clock_path_${MODE}.csv {
                PTC_report_different_clock_crossing ${REPORT_DIR}/clock_crossing_${MODE}.csv
            }
        }
    }
}
