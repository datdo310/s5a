...
proc ERROR_MESSAGE {{argv0}} {
    puts ""
    puts "    usage : $argv0 <timing_report>"
    puts ""
    exit
}
    ...
    switch $inREP {
        ...
        1 {
            if {[regexp {clock [0-9a-zA-Z_']* \(} $str] > 0} {
                ...
                set ST_TIME [lindex $str [expr [llength $str] - 1]]
                #if {$FlagTran == "true"} {
        ...
        2 {
            if {[regexp {clock [0-9a-zA-Z_']* \(} $str] > 0} {
                if {[regexp "source latency" $str] > 0} {
                    set ST_LATENCY [lindex $str [expr [llength $str] -1]]
                    #set ST_LATENCY [lindex $str 5]
                } else {
                    set ED_CLK  [lindex $str 1]
                    set ED_TIME [lindex $str [expr [llength $str] - 1]]
                    #if {$FlagTran == "true"} {
                ...
                if {[regexp {/[T]?CLK[AB]?} [lindex $str 0]] > 0} {
                    ...
                    set Latency_ST [lindex $str [expr [llength $str] - 2]]
                    #if {$FlagTran == "true"} {
        ...
        3 {
            if {[regexp "source latency" $str] > 0} {
                ...
                set ED_LATENCY [lindex $str [expr [llength $str] -1]]
            }
            if {[regexp "time borrowed from endpoint" $str] > 0} {
                ...
                set CRPR       [lindex $str [expr [llength $str] -2]]
                set Latency_ED [lindex $str [expr [llength $str] -1]]
