...
#### MAIN ###
set pin_pin {}
foreach_in_collection this_net [remove_from_collection [get_nets -h -top_net_of_hierarchical_group -segments ] [get_nets -hier {*Logic0* *Logic1*}]] {
if { [sizeof_collection [filter_collection [get_loads $this_net] "object_class==pin" ]] > 0 } {
append_to_collection pin_pin $this_net
}
}
sizeof_collection [filter_collection $pin_pin "has_valid_parasitics==false"]

redirect lump.net { puts "" }
foreach_in_collection net_name [filter_collection $pin_pin "has_valid_parasitics==false"] {
	redirect -append lump.net {	
